# Script: Create_BOMC_ISOs_from_XML_20210714.21.ps1
# Version: 20210714 rev 21
# 
# This script will:
...
$OneCliUrl = "http://download.lenovo.com/servers/mig/2021/06/16/54094/lnvgy_utl_lxce_onecli01n-3.2.0_winsrv_x86-64.zip"
$BomcUrl = "http://download.lenovo.com/servers/mig/2021/06/16/54099/lnvgy_utl_lxce_bomc01l-12.2.0_windows_i386.exe"
$UXVersion = "lnvgy_utl_lxce_ux_3.2.0_anyos_x86-64" #Note: must be anyos
$BomcBoot = "lnvgy_utl_lxce_bomc-1.0.0-2.0.0_anyos_32-64" #won't autodownload from bomc cli, but can download from bomcgui
...
$UtilPath = $TAPath + "\utils"
$UpdatesDLPath = $TAPath + "\UpdateStaging"
...
function CreateFolder ($folder) {
    ...
    if (Test-Path $folder) {
        write-host "Created" -fore Green
    } else {
        write-host "Failed to Create!" -fore Red
        break
    }
}
...
foreach ($PolicyFile in (Get-ChildItem $PoliciesPath | where {! $_.PSIsContainer})) {
    ...
    if ($haveUpdates) {
        $ISOpath = "$TAPath\" + $PolicyFile.BaseName + ".iso"
